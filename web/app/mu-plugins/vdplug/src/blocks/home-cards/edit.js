...
export default function Edit({ attributes, setAttributes }) {
    ...
    const addItem = () => {
        const newItems = [...(items || []), { icon: null, text1: '',text2: '' }];
                ...
                items.map((item, index) => (
                    <div key={index} className="card-items">
                        <MediaUpload
                            ...
                            render={({ open }) => (
                                <Button className='upload-icon img-box' onClick={open}>
                                    {item.icon ? (
                        ...
                        className='text'
                            ...
                            onChange={(text1) => updateItem(index, { text1 })}
                        />
                             <RichText
                        className='text'
                        tagName='p'
                            value={item.text2}
                            placeholder={ __('Enter list item content', 'repeater-list-with-icon')}
                            onChange={(text2) => updateItem(index, { text2 })}
                        />
