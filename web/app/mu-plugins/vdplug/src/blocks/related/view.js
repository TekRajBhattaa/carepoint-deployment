...
function displayResults(data, component) {
    const container = component.querySelector('.related__columns')
    let markup = ''

    data.forEach(row => {
        const featuredMedia = row._embedded["wp:featuredmedia"][0]
        markup += `<a class="teaser is-layout-flow wp-block-group" href="${row.link}"><article>`
...
function loadMoreHandler(ev) {
    ...
    const component = btn.closest('.related');
    // Display data
...
window.addEventListener('DOMContentLoaded', function () {
    const moreButtons = document.querySelectorAll('.related .loadmore')
